      ...
      <div className="flex flex-col md:flex-row items-center gap-4 sm:gap-6">
        <div className="relative flex-1 w-full">
           ...
           <input 
             ...
             className="w-full pl-11 pr-6 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 font-bold text-slate-900 shadow-sm transition-all text-xs sm:text-sm"
           />
        </div>
        <div className="flex items-center w-full md:w-auto bg-white border border-slate-200 rounded-xl p-1 shadow-sm shrink-0">
           ...
           <button 
             ...
             className={`flex-1 md:flex-none px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all whitespace-nowrap ${sortField === 'level' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400'}`}
           >
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                   <h3 className="text-sm font-black text-slate-900 truncate tracking-tight">{student.name}</h3>
                   {student.level && (
                     <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-[8px] font-black uppercase tracking-widest rounded border border-indigo-100">
                       {student.level}
                     </span>
                   )}
                </div>
                <div className="flex flex-wrap items-center gap-x-3 text-slate-400">
                  <div className="flex items-center space-x-1.5">
                    ...
                    <span className="text-[10px] font-bold text-slate-600 tracking-tight">{student.contact || t('students.no_contact')}</span>
                  </div>
                  <div className="hidden sm:flex items-center space-x-1.5 opacity-40">
                    <div className="w-0.5 h-0.5 bg-slate-400 rounded-full" />
                    <span className="text-[8px] font-mono tracking-widest uppercase font-bold">ID: {student.id.slice(0, 8)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2 shrink-0">
                ...
                <div className="flex items-center bg-slate-50 border border-slate-100 rounded-lg p-0.5">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  </button>
                  <button 
                    onClick={() => setConfirmDeleteId(student.id)} 
                    className="p-1.5 text-slate-200 hover:text-red-500 transition-all"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-md overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="px-8 py-6 border-b border-slate-50 bg-slate-50/30">
              <h3 className="text-xl font-black text-slate-900 tracking-tight">{editingStudent ? t('students.edit_profile') : t('students.registration')}</h3>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-5">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 font-bold transition-all text-slate-900"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 font-bold transition-all appearance-none text-slate-900"
                  >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="px-8 py-6 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-0.5">{t('students.manage_links')} <span className="text-indigo-600">{accessModalStudent.name}</span></p>
                </div>
                <button onClick={() => setAccessModalStudent(null)} className="p-2 text-slate-400 hover:text-slate-600 transition-colors">
                ...
                {isAccessLoading ? (
                      ...
                      <div className="w-full flex items-center justify-between px-4 py-3 bg-indigo-50 border border-indigo-100 rounded-xl">
                          <div className="flex items-center space-x-3">
                              ...
                              <div className="flex flex-col">
                                  <span className="text-[8px] font-black text-indigo-400 uppercase tracking-widest">{t('students.authorized_user_id')}</span>
                  ...
                  <div className="w-full space-y-6">
                      <div className="py-8 text-center bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('students.no_accounts')}</p>
                      </div>
                      
                      <div className="space-y-4">
                          <label className="block text-[9px] font-black text-indigo-400 uppercase tracking-widest ml-1">{t('students.invite_user')}</label>
                          <div className="flex gap-2">
                              ...
                              <button 
                                  ...
                                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl text-[9px] uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
                              >
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={() => setConfirmDeleteId(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-sm overflow-hidden p-8 animate-in fade-in zoom-in duration-300 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex space-x-3">
              ...
              <button onClick={handleDelete} disabled={isProcessing} className="flex-1 px-4 py-2.5 text-[10px] font-black text-white rounded-xl bg-red-600 hover:bg-red-700 uppercase tracking-widest transition-all active:scale-95">
