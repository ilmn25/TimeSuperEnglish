...
import { Booking, StudentGroupedData, Teacher, Student } from '../types';
...
const TeacherAttendance: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const bookings = await api.getAllBookings(profile.org_id!, { 
  ...
  const handleCheckIn = async (bookingId: string) => {
    ...
    try {
      ...
      await api.updateBooking(teacher.org_id!, bookingId, { check_in: now });
      loadData();
  ...
  const handleCheckOut = async (bookingId: string) => {
    ...
    try {
      const now = getHKTTimeString();
      await api.updateBooking(teacher.org_id!, bookingId, { check_out: now });
      loadData();
    } catch (err) { alert(t('common.error')); }
  };
  ...
  const handleManualAdd = async (bookingId: string, start: string, end: string) => {
    if (!teacher) return;
    try {
      await api.updateBooking(teacher.org_id!, bookingId, { check_in: start, check_out: end });
  ...
  const handleSaveNote = async () => {
    if (!teacher || !noteModalConfig) return;
    try {
      await api.updateBooking(teacher.org_id!, noteModalConfig.bookingId, { comment: noteModalConfig.comment });
  ...
  const handleClearAttendance = async (bookingId: string) => {
    if (!teacher) return;
    try {
      await api.updateBooking(teacher.org_id!, bookingId, { check_in: null, check_out: null });
      setManualModalConfig(null);
      loadData();
    } catch (err) { alert(t('common.error')); }
  };
